...
def generate_decode_key(source_logic: str, target_domain: str, mapping_json: str, final_operational_output: str) -> str:
    """
    Agent 4: The Decode Key Generator.
    Produces a side-by-side translation guide that maps every metaphorical element
    back to its real-world counterpart, making the operational output a complete teaching tool.
    """
    client = get_client()
    ...
    system_instruction = (
        ...
        "a metaphorical operational output back to the original real-world concept. "
        "This document should let a learner read the metaphor AND understand "
        "exactly what each element represents in reality."
    )

    prompt = (
        f"Original Source Logic:\n{source_logic}\n\n"
        f"Target Domain:\n{target_domain}\n\n"
        f"Structural Mapping Dictionary:\n{mapping_json}\n\n"
        f"Final Metaphorical Operational Output:\n{final_operational_output}\n\n"
        "Generate a DECODE KEY document in Markdown with these sections:\n\n"
        "1. **Quick Reference Table**: A two-column table mapping every translated entity/term "
        "to its real-world equivalent (e.g., 'The Herald → SYN Packet').\n\n"
        "2. **Annotated Summary**: A short paragraph (3-5 sentences) that re-states the core operational output "
