...
========================================================
Runs N source logics through both a Monolithic LLM and the 4-Agent IMAW
...
def run_monolithic(source_logic: str, target_domain: str, tracker: TelemetryTracker) -> str:
    """
    ...
    prompt = (
        ...
        f"The final output must be a operational output written 100% inside the metaphor — "
        ...
        f"3. Write the operational output using ONLY metaphorical terms.\n"
        f"4. Never mention any technical jargon from the source domain.\n"
        f"5. The operational output should be engaging, detailed, and structurally faithful.\n\n"
        f"Write the operational output now."
    )
...
def run_imaw_pipeline(source_logic: str, target_domain: str, tracker: TelemetryTracker) -> dict:
    """
    ...
    """
    start = time.time()
    result = IMAWOrchestrator.generate_operational_output(
        source_logic, target_domain, include_decode_key=False
    )
    latency = time.time() - start
...
def grade_for_leakage(source_logic: str, target_domain: str,
...
def calibrated_regrade(source_logic: str, target_domain: str,
...
**HARD LEAKAGE:** A specific source-domain technical term appears VERBATIM in the output.
  ...
  - These are unambiguous technical jargon that has NO natural place in the target domain domain.

**SOFT RESEMBLANCE (NOT leakage):** The output structurally mirrors the source logic's logic
but uses vocabulary that is either:
  - Generic English words natural to both domains (e.g., "system", "process", "cycle", "reduced")
  - Words that belong to the target domain domain even if they also appear in the source
...
2. A word is HARD only if it is specific technical jargon from the source domain that
   has no natural meaning in the target domain context.
...
def regrade_from_csv(csv_path: str, dry_run: bool = False):
    ...
    # Load the test corpus for source logic lookup
    ...
    for i, row in enumerate(rows, 1):
        ...
        source_logic = corpus_entry.get("source_logic", row.get("source", ""))
        metaphor = corpus_entry.get("target_domain", row.get("metaphor", ""))
...
def main():
    ...
    for i, case in enumerate(corpus, 1):
        print(f"\n── Concept {i}/{count}: {case['source_logic'][:60]}...")
        print(f"   Metaphor: {case['target_domain']}")

        # ── Run Monolithic ──
        print("   ⏳ Running monolithic pipeline...")
        try:
            mono_output = run_monolithic(case["source_logic"], case["target_domain"], tracker)
        except Exception as e:
        ...
        try:
            imaw_result = run_imaw_pipeline(case["source_logic"], case["target_domain"], tracker)
            imaw_output = imaw_result["operational_output"]
        except Exception as e:
            print(f"   ❌ IMAW pipeline failed: {e}")
            imaw_output = f"[ERROR: {e}]"
            imaw_result = {"operational_output": imaw_output, "abstract_schema": "", "mapping": ""}

        # ── Grade both ──
        print("   ⏳ Grading monolithic for leakage...")
        mono_grade = grade_for_leakage(
            case["source_logic"], case["target_domain"], mono_output, "monolithic", tracker
        )

        print("   ⏳ Grading IMAW for leakage...")
        imaw_grade = grade_for_leakage(
            case["source_logic"], case["target_domain"], imaw_output, "imaw", tracker
        )
        ...
        results.append({
            ...
            "source": case["source_logic"][:80],
            "metaphor": case["target_domain"],
            # Monolithic
