...
def get_chat_response(user_question: str, source_logic: str, target_domain: str, mapping_json: str) -> dict:
    """
    ...
    reverse_prompt = (
        ...
        f"Here is the established mapping dictionary between Abstract Schema -> Target Domain:\n"
        f"```json\n{mapping_json}\n```\n\n"
        f"The user is immersed in the target domain ('{target_domain}') and just asked this question:\n"
    ...
    oracle_prompt = (
        ...
        f"Here is the absolute factual truth regarding the original source logic:\n"
        f"```text\n{source_logic}\n```\n\n"
        f"Based ONLY on the mechanics defined in the source logic above, answer the following abstract question accurately:\n"
        f"ABSTRACT QUESTION: \"{abstract_question}\"\n\n"
        f"Task: Provide the pure technical answer. If the answer is not explicitly detailed in the source logic, "
    ...
    forward_prompt = (
        f"You are a State-Bound Query Engine.\n\n"
        f"Here is the established mapping dictionary (Abstract Schema -> Target Domain):\n"
        f"```json\n{mapping_json}\n```\n\n"
        f"The user is operating under this Target Domain: {target_domain}\n"
        ...
        f"but strictly conforms to the Target Domain. Use the mapping dictionary to ensure structural fidelity. "
        f"NEVER break character. NEVER reference real-world technology, architecture, businesses, etc. "
        f"You are an operational query engine strictly bound by the approved JSON state. Answer queries using only the translated domain vocabulary, referencing the Decode Key for traceability."
    )
...
def detect_out_of_schema(technical_answer: str, abstract_schema: str, source_logic: str) -> dict:
    """
    ...
    prompt = (
        ...
        f"```json\n{abstract_schema}\n```\n\n"
        f"Here is the original source logic:\n"
        f"```text\n{source_logic}\n```\n\n"
...
def expand_schema(new_source_material: str, existing_abstract_schema: str,
                   existing_mapping_json: str, target_domain: str) -> dict:
    """
    ...
    # Step 2: Map the new abstract entities into the target domain,
